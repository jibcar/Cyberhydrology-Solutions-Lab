...
## Revision on 2024-02-14
## while calculating std deviation, used drop na - math.sqrt(statistics.variance(discharge_data.dropna()))

## Revision on 2023-07-03
## Added missing value computation and analysis start and end year

## Revision on 2023-06-08
## Added calulation of std deviation just after trend slope
...
start_year='1950' #Options ('1950','1981')
end_year='2014' #Options ('2014','2021')

desire_year_length=30
percent_miss=5 #percentage

## Path for Input and Output
FolderName1="./USGS_ProcessedData"
FolderName2=f"./USGS_Results/z_Result10f_{start_year}_{end_year}"


length_data_required=math.ceil((100-percent_miss)/100*(int(end_year)-int(start_year)+1))
length_missing_allowed=math.floor(percent_miss/100*(int(end_year)-int(start_year)+1))
...
def ADF_test_f(timeseries):
    ...
    adftest = adfuller(x=timeseries.dropna(),
                       maxlag=1,
                       regression='c')
...
def PP_test_f(timeseries):
    ...
    pptest=PhillipsPerron(timeseries.dropna(),
                          lags=1,
                          trend='c',
                          test_type='tau')
...
def MK_test_f(timeseries):
    ...
    return(original_mktest[0],hr_modified_mktest[0],
           original_mktest[1],hr_modified_mktest[1],
           original_mktest[2],hr_modified_mktest[2],
               original_mktest[3],hr_modified_mktest[3],
               original_mktest[4],hr_modified_mktest[4],
               original_mktest[5],hr_modified_mktest[5],
               original_mktest[6],hr_modified_mktest[6],
               original_mktest[7],hr_modified_mktest[7],
               original_mktest[8],hr_modified_mktest[8])
...
Folder_MissingData=FolderName2+"/MissingData"
...
if os.path.exists(Folder_StationarityResults) == True:
    ...
    shutil.rmtree(Folder_MissingData)
    os.mkdir(Folder_MissingData)
...
for key in state_list_dict:   #{'in':'Indiana'}:#
    ...
    stations = pd.read_csv(FolderName_StateList+'/Data_' + state_code + '_raw'  + '.txt', 
                           header=0,delimiter="\t",
                           comment='#',na_values=['Eqp'],
                           converters={'site_no': lambda x: str(x)},lineterminator='\n')
    ...
    #colNames for raw data
    # define column names
    colNames2=['agency_cd','site_no','peak_dt', 'peak_tm','peak_va','peak_cd','gage_ht','gage_ht_cd','year_last_pk','ag_dt','ag_tm','ag_gage_ht','ag_gage_ht_cd']
    ...
    result_data = (('SlNo\tUSGS_Station\tStation_Name\tState_Name\tStation_Lat\tStation_Lon\tDrain_Area\tCountyCode\tState_Code'
                    '\tHUC_Code\tBasin_Code\tPeakBeginDate\tPeakEndDate\tUSGS_Count\tCalc_Count'+
                    '\tACF_Lag1\tACF_Lag2'+
                    '\tOri_Test_Trend\tOri_Test_h\tOri_Test_pvalue\tOri_Test_z\tOri_Test_tau\tOri_Test_score\tOri_Test_var_s\tOri_Test_slope\tOri_Test_intercept'+
                    '\tHR_Modif_Trend\tHR_Modif_h\tHR_Modif_pvalue\tHR_Modif_z\tHR_Modif_tau\tHR_Modif_score\tHR_Modif_var_s\tHR_Modif_slope\tHR_Modif_intercept'+
                    '\tMK_Combi_Trend\tMK_Combi_h\tMK_Combi_pvalue\tMK_Combi_z\tMK_Combi_tau\tMK_Combi_score\tMK_Combi_var_s\tMK_Combi_slope\tMK_Combi_intercept'+
                    '\tStdDev\tCP_p-value\tCP_decision\tChange_Point\tChange_Point_corr\tChange_Point_year'+
                    '\tADF_pvalue\tKPSS_pvalue\tPP_pvalue\tADF_Result\tKPSS_Result\tPP_Result\tStationarity_Scenario\tStationarity_MAR'+
                    '\tVar_2Divide\tVar_2Divide_First\tVar_2Divide_Second\tVar_2Divide_Comparison\tVar_3Divide\tVar_CPDivide\tVar_CPDivide_First\tVar_CPDivide_Second\tVar_CPDivide_Comparison'+
                    '\tSW_Result\tNorm_Result\tAD_Result\tNormality_Scenario\tNormality_Decision\tStationarity_AIN\tRemark')+'\n')
    
    missing_data = (f'SlNo\tCount_j\tUSGS_Station\tStation_Name\tState_Name\tStation_Lat\tStation_Lon\tDrain_Area\tCountyCode\tState_Code\tHUC_Code\tBasin_Code\tPeakBeginDate\tPeakEndDate\tUSGS_Count\tCount_Total\tMissing_in_Total\tCount_in_{start_year}_{end_year}\tMissing_in_{start_year}_{end_year}\tIncludedInAnalysis\tLength_Required'+'\n')
    ...
    for j in range(len_rows):
        ...
        print(f'Site Num:\t {station_id} in state:\t {state_code}')
        ...
        discharge_data_old=station_data_ts['Discharge']
        
        #print(f'Length of discharge data :\t{len(discharge_data_old)}')
        
        if len(discharge_data_old)>0:
            stations_low_data.append(station_id)
            #print("Station with 0 rows:\t",station_id)
            ...
            DataDF = pd.read_csv(FolderName_PeakFlowData_Statewise+'/Data_'+str(station_id)+'_raw.txt',header=1, 
                                 names=colNames2,
                                 delimiter='\t',parse_dates=[2], 
                                 comment='#',
                                 na_values=['Eqp'])
            DataDF['index']=(DataDF['peak_dt'])
            #DataDF['index']=pd.to_datetime(DataDF['peak_dt'])
            DataDF = DataDF.set_index('index')
            ...
            MissingValues = DataDF['peak_va'].isna().sum()
            #print("Missing Values:",MissingValues)
            ## Remove invalid streamflow data
            #DataDF.peak_va[(DataDF['peak_va']<0)]=np.nan
            #print(len(DataDF.peak_va[start_year:end_year]))
            #print((DataDF.peak_va[start_year:end_year]))
            
            #print(int(end_year)-int(start_year)+1)
            MissingValues2 = DataDF['peak_va'].isna().sum()
            #print("Missing Values2:",MissingValues2)
            
            MissingValues2=0
            discharge_data2=DataDF.peak_va[start_year:end_year]
            MissingValues2 = discharge_data2.isna().sum()
            #print("Missing Values2:",MissingValues2)
            
            #DataDF = DataDF['peak_va'].dropna()
            
            #f'SlNo\tUSGS_Station\tMissing_Total\tMissing_{start_year}_{end_year}'+'\n'
            
            
            #print("SlNo:",j,"--","USGS:",station_id,"--","Missing V:",MissingValues,"Missing V2:",MissingValues2,len(DataDF.peak_va[start_year:end_year]),(int(end_year)-int(start_year)+1))
            missing_newline = '\t'.join([str(sl_num), str(j), str(station_id),station_name,state_code,str(station_lat),str(station_lon),str(station_drain_area),
                                  ...
                                  str(len(discharge_data_old)),str(MissingValues),str(len(DataDF.peak_va[start_year:end_year])),str(MissingValues2),
                                  str(MissingValues2<=length_missing_allowed and (int(end_year)-int(start_year)+1)==(len(DataDF.peak_va[start_year:end_year]))),
                                  str((int(end_year)-int(start_year)+1))])
            
            missing_data+= missing_newline + '\n'
    
            #if (len(discharge_data)>=desire_year_length):
            #if (MissingValues2<=length_missing_allowed and (len(DataDF.peak_va[start_year:end_year]))>=length_data_required):
            if (MissingValues2<=length_missing_allowed and 
                #(len(DataDF.resample('AS-SEP').agg({'peak_va': 'max'}).peak_va[start_year:end_year]))>=length_data_required):
                (len(DataDF.peak_va[start_year:end_year]))>=length_data_required):
                print("OK")
                #DataDF.drop(index=[0],inplace=True)
                #DataDF.reset_index(drop=True)
                
                ## By default (DataDF.reset_index(drop=True)) doesn’t update the existing DataFrame, you can use inplace=True to update the current DataFrame.
    
                #DataDF.reset_index(drop = True, inplace=True)
                
                #discharge_data=DataDF.resample('AS-SEP').agg({'peak_va': 'max'})[start_year:end_year]['peak_va']
                discharge_data=DataDF[start_year:end_year]['peak_va']
                ...
                ## Trend test
                (original_mk_trend, 
                hr_modified_mk_trend,
                original_mk_h, 
                hr_modified_mk_h,
                original_mk_pvalue, 
                hr_modified_mk_pvalue,
                original_mk_z, 
                hr_modified_mk_z,            
                original_mk_tau, 
                hr_modified_mk_tau, 
                original_mk_score, 
                hr_modified_mk_score,
                original_mk_var_s, 
                hr_modified_mk_var_s, 
                original_mk_slope, 
                hr_modified_mk_slope,
                original_mk_intercept, 
                hr_modified_mk_intercept)=MK_test_f(discharge_data)
                
                if lag1_acf=='insignificant':
                    mk_trend=original_mk_trend
                    mk_h=original_mk_h
                    mk_pvalue=original_mk_pvalue
                    mk_z=original_mk_z
                    
                    mk_tau=original_mk_tau
                    mk_score=original_mk_score
                    mk_var_s=original_mk_var_s
                    mk_slope=original_mk_slope
                    mk_intercept=original_mk_intercept
                elif lag1_acf=='significant':
                    mk_trend=hr_modified_mk_trend
                    mk_h=hr_modified_mk_h
                    mk_pvalue=hr_modified_mk_pvalue
                    mk_z=hr_modified_mk_z
                    
                    mk_tau=hr_modified_mk_tau
                    mk_score=hr_modified_mk_score
                    mk_var_s=hr_modified_mk_var_s
                    mk_slope=hr_modified_mk_slope
                    mk_intercept=hr_modified_mk_intercept
                else:
                    mk_trend='check error'
                    mk_h='check error'
                    mk_pvalue='check error'
                    mk_z='check error'
                    
                    mk_tau='check error'
                    mk_score='check error'
                    mk_var_s='check error'
                    mk_slope='check error'
                    mk_intercept='check error'
                ...
                newline2 = '\t'.join([str(sl_num),str(station_id),station_name,state_code,str(station_lat),str(station_lon),str(station_drain_area),
                                      ...
                                      original_mk_trend, str(original_mk_h), str(original_mk_pvalue), str(original_mk_z), 
                                      str(original_mk_tau),str(original_mk_score),str(original_mk_var_s),str(original_mk_slope), str(original_mk_intercept),
                                      hr_modified_mk_trend, str(hr_modified_mk_h), str(hr_modified_mk_pvalue), str(hr_modified_mk_z), 
                                      str(hr_modified_mk_tau),str(hr_modified_mk_score),str(hr_modified_mk_var_s),str(hr_modified_mk_slope),str(hr_modified_mk_intercept),
                                      mk_trend, str(mk_h),str(mk_pvalue), str(mk_z), 
                                      str(mk_tau),str(mk_score),str(mk_var_s),str(mk_slope), str(mk_intercept),
                                      str(math.sqrt(statistics.variance(discharge_data.dropna()))),
                ...
                #print("XXXXXXXXXless than 50",i," - ",": ", stations['station_nm'][j],station_name," - ",len(discharge_data))
                #Added line below
                continue
    ...
    ## Converts missing_data from str class to bytes class
    missing_data2 = missing_data.encode()
    with open(Folder_MissingData+'/A_'+state_code+'_Missing_Data' + '.txt', 'wb') as f3:
        f3.write(missing_data2)
    f3.close
    
    
    ## Converts multi-line string that contains the column names
    data_missing = StringIO(missing_data) ##needs StringIO
    data_missing_df = pd.read_csv(data_missing, 
                     sep="\t")
    
    ## Converts multi-line string that contains the column names
    data_result = StringIO(result_data) ##needs StringIO
    data_result_df = pd.read_csv(data_result, 
                     sep="\t")
...
with open(f'{FolderName2}/B_Results_Trend_Stationarity_{start_year}_{end_year}_missing_{percent_miss}percent_alpha_{alpha}_{datetime.now().strftime("%Y%m%d-%H%M%S")}.txt', "wb") as outfile:
