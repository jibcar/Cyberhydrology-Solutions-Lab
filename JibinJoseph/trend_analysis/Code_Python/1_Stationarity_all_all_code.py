...
start_year='all' #Options ('all')
end_year='all' #Options ('all')
    ...
    for j in range(len_rows):
        ...
        if (len(discharge_data)>=desire_year_length):
            ...
            newline2 = '\t'.join([str(sl_num),str(station_id),station_name,state_code,str(station_lat),str(station_lon),str(station_drain_area),
                                  ...
                                  mk_result, str(mk_pvalue), str(mk_z), str(mk_slope), str(math.sqrt(statistics.variance(discharge_data))),
...
with open(f'{FolderName2}/B_Results_Trend_Stationarity_{start_year}_{end_year}_missing_NA_percent_alpha_{alpha}_{datetime.now().strftime("%Y%m%d-%H%M%S")}.txt', "wb") as outfile:
